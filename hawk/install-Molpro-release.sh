...
# configuration
molpro_version=2020.1.2 # check https://www.molpro.net/download/ to see what's available
...
module load git # need git version 1.9 or higher
eigen_version=3.3.7
...
./autogen.sh
./configure FC=mpif90 CXX=mpicxx CC=mpicc --with-mpi-pr --prefix=$working_directory --with-blas=no --with-lapack=no --with-scalapack=no --disable-f77
...
majorminor=$(echo $molpro_version|sed -e 's/\.[0-9]*$//')
